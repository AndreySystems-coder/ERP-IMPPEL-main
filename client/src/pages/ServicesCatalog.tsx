...
export default function ServicesCatalog() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editing) {
        ...
        toast({ title: "ServiÃ§o atualizado com sucesso!" });
      } else {
        await createService.mutateAsync(payload);
        toast({ title: "ServiÃ§o criado com sucesso!" });
      }
      setIsOpen(false);
      resetForm();
    } catch (err: any) {
  ...
  const handleDelete = async (id: number, serviceName: string) => {
    if (!confirm(`Excluir "${serviceName}"? Esta aÃ§Ã£o nÃ£o pode ser desfeita.`)) return;
    try {
      await deleteService.mutateAsync(id);
      toast({ title: "ServiÃ§o excluÃ­do." });
    } catch (err: any) {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Gerencie serviÃ§os, descriÃ§Ãµes, preÃ§os e materiais necessÃ¡rios por obra.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Button onClick={openNew} data-testid="button-new-service">
            <Plus className="w-4 h-4 mr-2" /> Novo ServiÃ§o
          </Button>
        </div>
      </div>
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 text-slate-400">
              ...
              <p className="font-medium">Nenhum serviÃ§o encontrado</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="border-b-2 border-slate-200">
                    <th className="text-left py-3 px-3 font-semibold text-slate-600 uppercase text-xs tracking-wider">ServiÃ§o</th>
                    <th className="text-left py-3 px-3 font-semibold text-slate-600 uppercase text-xs tracking-wider">DescriÃ§Ã£o</th>
                    <th className="text-right py-3 px-3 font-semibold text-slate-600 uppercase text-xs tracking-wider">R$/mÂ²</th>
                    <th className="text-right py-3 px-3 font-semibold text-slate-600 uppercase text-xs tracking-wider">MÃ£o obra/mÂ²</th>
                    ...
                    return (
                      <tr key={service.id} className="hover:bg-slate-50 transition-colors" data-testid={`row-service-${service.id}`}>
                        ...
                        <td className="py-3 px-3 text-slate-500 max-w-[220px]">
                          <span className="line-clamp-2 text-xs">{service.description || "â€”"}</span>
                        </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-5 py-2">
          <Input
            ...
            required
            placeholder="Ex: Manta asfÃ¡ltica poliÃ©ster 3mm"
            data-testid="input-service-name"
          />

          <div>
            <label className="text-sm font-semibold text-slate-700 block mb-1">
              DescriÃ§Ã£o <span className="font-normal text-slate-400 text-xs">(aparece no PDF do orÃ§amento)</span>
            </label>
            <textarea
              value={description}
              ...
              placeholder="Descreva o serviÃ§o detalhadamente..."
              data-testid="input-service-description"
            />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Input label="PreÃ§o de venda (R$/mÂ²) *" type="number" step="0.01" min="0" value={pricePerUnit} onChange={e => setPricePerUnit(e.target.value)} placeholder="0,00" data-testid="input-service-price" />
            <Input label="MÃ£o de obra (R$/mÂ²)" type="number" step="0.01" min="0" value={laborCostPerM2} onChange={e => setLaborCostPerM2(e.target.value)} placeholder="0,00" data-testid="input-service-labor" />
            <Input label="Material (kg ou L / mÂ²)" type="number" step="0.01" min="0" value={materialConsumptionPerM2} onChange={e => setMaterialConsumptionPerM2(e.target.value)} placeholder="0,00" data-testid="input-service-material" />
            <Input label="Transporte (R$/mÂ²)" type="number" step="0.01" min="0" value={transportCostPerM2} onChange={e => setTransportCostPerM2(e.target.value)} placeholder="0,00" data-testid="input-service-transport" />
          </div>
            ...
            <div className="bg-emerald-50 border-b border-emerald-100 px-4 py-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-emerald-600 font-normal bg-emerald-100 px-2 py-0.5 rounded-full ml-1">
                  uso interno â€” nÃ£o aparece no PDF
                </span>
              </div>
              <button
                type="button"
            ...
            <div className="p-4 space-y-3">
              {serviceMaterials.length === 0 ? (
                <div className="text-center py-6 text-slate-400">
                  ...
                  <p className="text-xs mt-1">Clique em "Adicionar Material" para vincular itens do estoque e definir o consumo por mÂ² ou fixo.</p>
                </div>
              ) : (
                <>
                    ...
                    return (
                      ...
                      >
                          ...
                          >
                            ...
                            {(inventoryItems as any[]).sort((a: any, b: any) => a.name.localeCompare(b.name)).map((item: any) => (
                              <option key={item.id} value={item.id}>
                                {item.name} â€” {item.unit || "unid"} (estoque: {item.quantity})
                              </option>
                            ))}
                          </select>
                          {invItem && (
                        ...
                        <div className="space-y-2.5">
                          ...
                          {mat.unit === "per_m2" && (
                            <div>
                              <label className="text-xs text-slate-500 font-medium block mb-1">Unidades por mÂ²</label>
                              <input
                                type="number"
                                min="0.001"
                                step="0.001"
                          ...
                          {(mat.unit === "per_kg" || mat.unit === "fixed") && (
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                <label className="text-xs text-slate-500 font-medium block mb-1">Kilos por mÂ²</label>
                                <input
                                  type="number"
                                  min="0.001"
                                  step="0.001"
                                ...
                                />
                                <p className="text-xs text-slate-400 mt-0.5">kg consumidos por mÂ²</p>
                              </div>
                              <div>
                        ...
                        {mat.inventoryId > 0 && mat.unit === "per_m2" && qty > 0 && (
                          <div className="bg-slate-50 rounded-lg px-3 py-2 text-xs text-slate-600 flex items-center justify-between">
                            ...
                            <div className="flex gap-4">
                              {[20, 50, 100].map(area => (
                                <span key={area}>
                                  <strong>{area} mÂ²</strong> â†’ {Math.ceil(qty * area)} <span className="text-slate-400">{invItem?.unit || "unid"}</span>
                                </span>
                              ))}
                            </div>
                          </div>
                        ...
                        {mat.inventoryId > 0 && (mat.unit === "per_kg" || mat.unit === "fixed") && Number(mat.kilosPerM2) > 0 && Number(mat.weightPerUnit) > 0 && (
                          <div className="bg-orange-50 border border-orange-100 rounded-lg px-3 py-2 text-xs text-orange-800">
                            <div className="font-semibold mb-1">CÃ¡lculo: (Ãrea Ã— {mat.kilosPerM2} kg/mÂ²) Ã· {mat.weightPerUnit} kg/unid</div>
                            <div className="flex gap-4 text-orange-700">
                              {[10, 20, 50, 100].map(area => {
                                ...
                                return (
                                  <span key={area}>
                                    <strong>{area} mÂ²</strong> â†’ {r} <span className="opacity-70">{invItem?.unit || "unid"}</span>
                                  </span>
                                );
                              })}
                            </div>
                  ...
                  {serviceMaterials.some(m => {
                    ...
                    <div className="flex items-center gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                      ...
                      Linhas sem material ou parÃ¢metros incompletos serÃ£o ignoradas ao salvar.
                    </div>
                  )}

                  {/* Preview table for 100mÂ² */}
                  {validMaterialsForPreview.length > 0 && (
                    <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3">
                      <p className="text-xs font-bold text-emerald-800 mb-2">
                        Consumo estimado para 100 mÂ²:
                      </p>
                      <div className="space-y-1">
                        {validMaterialsForPreview.map((m, i) => {
                          ...
                          const label = m.unit === "per_kg"
                            ? `(${m.kilosPerM2} kg/mÂ² Ã· ${m.weightPerUnit} kg/unid)`
                            : m.unit === "per_m2" ? `(${m.quantity}/mÂ²)` : "(fixo)";
                          return (
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button type="submit" className="flex-1" isLoading={isPending} disabled={!name.trim()} data-testid="button-save-service">
              {editing ? "Salvar AlteraÃ§Ãµes" : "Criar ServiÃ§o"}
            </Button>
          </div>
        </form>
      </Modal>
