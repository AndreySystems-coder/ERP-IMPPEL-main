  ...
  return (
    ...
    >
      <div className="px-3 py-2.5" ref={measureRef}>
        {title && (
          <div className="mb-2 font-medium text-popover-foreground text-xs">
            {title}
          </div>
        )}
        <div className="space-y-1.5">
          {rows.map((row) => (
            ...
            >
              ...
              <span className="font-medium text-popover-foreground text-sm tabular-nums">
  ...
  const handleClick = useCallback(
    (event: React.MouseEvent<SVGGElement>) => {
      ...
      for (let i = 0; i < domain.length; i++) {
        ...
        const bandStart = xScale(cat) ?? 0;
        const bandEnd = bandStart + bandWidth;
        const inBand = isHorizontal
          ? chartY >= bandStart && chartY <= bandEnd
          : chartX >= bandStart && chartX <= bandEnd;
        if (inBand) {
          onBarClick(data[i]!, i);
          break;
        }
    ...
    [onBarClick, xScale, bandWidth, isHorizontal, margin, data],
  );
    ...
    <BarChartProvider value={contextValue}>
      <svg aria-hidden="true" height={height} width={width}>
        ...
        <g
          ...
          style={{
            cursor: isLoaded ? (onBarClick ? "pointer" : "crosshair") : "default",
            touchAction: "none",
          }}
