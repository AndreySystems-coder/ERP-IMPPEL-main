...
export type ErpPdfReportType = BackupType | "movimentacoes" | "backup-completo";

export type ErpPdfImportPreview = {
...
async function extractPdf(file: File) {
  ...
  return { items, rows: groupRows(items), rawText };
}

function detectReport(rawText: string): { type: ErpPdfReportType; restoreType?: BackupType; title: string; headerTotal: number } {
  const normalized = normalizeText(rawText);
  if (!normalized.includes("imppel erp") || !normalized.includes("estrutura erp")) {
    throw new Error("Este arquivo não parece ser um relatório gerado pelo ERP IMPPEL.");
  }
  const found = REPORT_TYPES.find(report => normalized.includes(report.marker));
  if (!found) {
    throw new Error("Relatório do ERP reconhecido, mas o tipo não pôde ser identificado com segurança.");
  }
  const headerTotal = Number(rawText.match(/Total de registros:\s*(\d+)/i)?.[1] || 0);
  const title = rawText.split("\n").find(line => normalizeText(line).includes("imppel erp")) || `Relatório ${found.type}`;
  return { ...found, title, headerTotal };
}

function basePreview(report: ReturnType<typeof detectReport>, rawText: string): ErpPdfImportPreview {
  return {
...
function parseUsers(report: ReturnType<typeof detectReport>, rows: PdfRow[], rawText: string): ErpPdfImportPreview {
  const preview = basePreview(report, rawText);
  const users: any[] = [];
  const roles: any[] = [];
  ...
  for (const [label, name] of Object.entries(ROLE_TECHNICAL_BY_LABEL)) {
    ...
    seenRoles.add(name);
    const permissionsMatch = rawText.match(new RegExp(`${name}\\s+([^\\n\\f]+)`, "i"));
    const permissions = Object.fromEntries(
      String(permissionsMatch?.[1] || "")
        .split(",")
...
function parseProducts(report: ReturnType<typeof detectReport>, rows: PdfRow[], rawText: string): ErpPdfImportPreview {
  const preview = basePreview(report, rawText);
  const products: any[] = [];
  const seen = new Set<string>();
  for (const row of rows) {
    const name = cell(row, 35, 260);
    const category = cell(row, 260, 415);
    const brand = cell(row, 415, 500);
    const unit = cell(row, 500, 555);
    const salePrice = cell(row, 555, 660);
    const maxDiscount = cell(row, 660, 735);
    const active = cell(row, 735, 820);
    ...
    products.push({
      ...
      commission: 0,
      maxDiscount: parsePercent(maxDiscount),
      active: active !== "Não",
    });
...
function parseServices(report: ReturnType<typeof detectReport>, rows: PdfRow[], rawText: string): ErpPdfImportPreview {
  const preview = basePreview(report, rawText);
  const services: any[] = [];
  ...
  const flush = () => {
    ...
    if (!current.name || !current.pricePerUnit) {
      ...
      services.push({
        ...
        laborCostPerM2: current.laborCostPerM2 || 0,
        transportCostPerM2: current.transportCostPerM2 || 0,
        materialConsumptionPerM2: 0,
  ...
  for (const row of rows) {
    ...
    if (startsService) {
      ...
      current = {
        ...
        pricePerUnit: parseMoney(priceText),
        laborCostPerM2: parseMoney(laborText),
        transportCostPerM2: parseMoney(transportText),
      };
      continue;
    }
    if (!current) continue;
...
function parseStock(report: ReturnType<typeof detectReport>, rows: PdfRow[], rawText: string): ErpPdfImportPreview {
  const preview = basePreview(report, rawText);
  const items: any[] = [];
  const movements: any[] = [];
  ...
  for (const row of rows) {
    ...
    if (/Histórico de Movimentações|Movimentações/i.test(rowText)) {
      inMovements = true;
      continue;
    }
  ...
  preview.backup = {
    ...
    preserveItemQuantitiesAfterMovements: report.type === "estoque",
...
export async function previewErpPdfReport(file: File): Promise<ErpPdfImportPreview> {
  const { rows, rawText } = await extractPdf(file);
  const report = detectReport(rawText);
  if (report.type === "usuarios") return parseUsers(report, rows, rawText);
  if (report.type === "produtos") return parseProducts(report, rows, rawText);
  if (report.type === "servicos") return parseServices(report, rows, rawText);
  if (report.type === "estoque" || report.type === "movimentacoes") return parseStock(report, rows, rawText);
  return {
    ...basePreview(report, rawText),
    ignoredCount: 1,
    ignored: ["Backup completo em PDF não é fonte técnica restaurável. Use o JSON/ZIP do Backup Completo."],
    warnings: ["PDF reconhecido, mas backup completo deve ser restaurado pelo arquivo técnico JSON/ZIP."],
  };
}
